...
struct ProgramState {
    ...
    glm::vec3 dirLightDir = glm::vec3(-0.3f, -1.0f, -0.35f);
    glm::vec3 dirLightAmbDiffSpec = glm::vec3(0.05f, 0.1f,0.1f);
...
int main() {
    ...
    //cave
    Model caveModel("resources/objects/cave/model.obj",true);
    caveModel.SetShaderTextureNamePrefix("material.");

    //stub
    Model stubModel("resources/objects/stub/golden_column.obj",true);
    stubModel.SetShaderTextureNamePrefix("material.");

    //flower
    Model flowerModel("resources/objects/flower/Flower.obj",true);
    flowerModel.SetShaderTextureNamePrefix("material.");

    //pumpkin
    Model pumpkinModel("resources/objects/pumpkin/pumpkin.obj",true);
    pumpkinModel.SetShaderTextureNamePrefix("material.");

    //Bloom_____________________________________________________________________________________________
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ourShader.setVec3("pointLights[0].position", glm::vec3(-2.9f ,1.0f+sin(time*2.0f)/4.0f, -3.5f));
        ...
        ourShader.setVec3("pointLights[1].position", glm::vec3(1.2f+ sin(time) ,2.0f+sin(time)/4.0f, -3.8f+cos(time)));
        ...
        //draw cave
        glm::mat4 modelCave = glm::mat4(1.0f);
        modelCave = glm::translate(modelCave,glm::vec3(0.0f,4.0f,0.0f));
        modelCave = glm::scale(modelCave, glm::vec3(0.25f));
        ourShader.setMat4("model", modelCave);
        caveModel.Draw(ourShader);
        //draw stub
        glm::mat4 modelStub = glm::mat4(1.0f);
        modelStub = glm::translate(modelStub,glm::vec3(3.15f,1.0f,-6.05f));
        modelStub = glm::scale(modelStub, glm::vec3(0.002));
        modelStub = glm::rotate(modelStub,glm::radians(-40.0f), glm::vec3(0.0f ,1.0f, 0.0f));
        modelStub = glm::rotate(modelStub,glm::radians(-7.8f), glm::vec3(0.0f ,0.0f, 1.0f));
        ourShader.setMat4("model", modelStub);
        stubModel.Draw(ourShader);
        //draw stub2
        glm::mat4 modelStub2 = modelStub;
        modelStub2 = glm::translate(modelStub2,glm::vec3(100.0f,0.0f,100.0f));
        ourShader.setMat4("model", modelStub2);
        stubModel.Draw(ourShader);
        //draw stub3
        glm::mat4 modelStub3 = modelStub2;
        modelStub3 = glm::translate(modelStub3,glm::vec3(100.0f,0.0f,100.0f));
        ourShader.setMat4("model", modelStub3);
        stubModel.Draw(ourShader);
        //draw stub4
        glm::mat4 modelStub4 = modelStub3;
        modelStub4 = glm::translate(modelStub4,glm::vec3(100.0f,0.0f,100.0f));
        ourShader.setMat4("model", modelStub4);
        stubModel.Draw(ourShader);
        //draw flower
        glm::mat4 modelFlower = glm::mat4(1.0f);
        modelFlower = glm::translate(modelFlower,glm::vec3(-2.9f,0.95f,-3.5f));
        modelFlower = glm::scale(modelFlower, glm::vec3(0.01));
        modelFlower = glm::rotate(modelFlower,glm::radians(50.0f), glm::vec3(0.0f ,1.0f, 0.0f));
        ...
        modelPumpkin = glm::rotate(modelPumpkin,glm::radians(90.0f), glm::vec3(1.0f ,0.0f, 0.0f));
...
void DrawImGui(ProgramState *programState) {
    ...
    ImGui::NewFrame();
    {
        ...
        ImGui::DragFloat3("Dirlight direction", (float*)&programState->dirLightDir, 0.05, -20.0, 20.0);

        ImGui::Text("Ambient      Diffuse      Specular");
        ImGui::DragFloat3("Diright settings", (float*)&programState->dirLightAmbDiffSpec, 0.05, 0.001, 1.0);
